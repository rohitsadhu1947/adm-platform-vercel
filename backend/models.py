...
# ---------------------------------------------------------------------------
# Bot State (Telegram bot persistence for Vercel serverless)
# ---------------------------------------------------------------------------
class BotState(Base):
    """Stores python-telegram-bot persistence data in PostgreSQL.

    Keys follow this pattern:
      - "user_data:{user_id}" - per-user conversation state (JSONB)
      - "chat_data:{chat_id}" - per-chat data (JSONB)
      - "conversations:{handler_name}" - ConversationHandler state map (JSONB)
      - "bot_data" - global bot data (JSONB)
      - "callback_data" - inline keyboard callback data (JSONB)
    """
