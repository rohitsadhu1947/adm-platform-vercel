...
async def version_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Show bot version — useful to verify deployment."""
    import datetime
    text = (
        f"{E_GEAR} <b>ADM Bot Version</b>\n\n"
        f"Version: <code>3.0.0-vercel</code>\n"
        f"Platform: <code>Vercel Serverless + Webhook</code>\n"
        f"Server time: <code>{datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}</code>\n"
        f"API: <code>{config.API_BASE_URL}</code>\n"
        f"Persistence: <code>PostgreSQL (Neon)</code>"
    )
